...
\revised{The aim of this dissertation is to develop a set of computational tools
  to expedite the design process of physics-based controllers that can execute
...
% First: falling and landing motions
\revised{The first goal is to develop falling and landing controllers for
  ...
  Similarly, safe falling and landing controllers are necessary for humanoids
  to protect fragile body parts and joints.}

% Second: general learning
\revised{Secondly, we want to develop a set of general computational tools for
  ...
  which are not well-understood unlike other motions such as running or
  reaching. 
  Instead, agile motions cover a wide range of diverse skills, such as jumping,
  vaulting, or rolling which are governed by different control mechanisms and
  principles.}  

% Third: export
\revised{The final topic is to design a new algorithm that help us to deploy a
  control policy from simulation to  physical system by learning the
  discrepancy between virtual and real systems.
  Although tools from the second challenge help us to easily develop
  controllers in simulation, simple agile motion trajectories that are
  developed in simulation are less likely to work on physical systems
...
humanoids. 
\revised{In Chapter 3, we discuss falls with the airborne phases for a virtual
  ...
  However, it relies on the manually designed rolling controller, and does not
  explicitly minimize the joint stress of virtual characters. 
  ...
  that reduces the maximum impact to body parts of robots, rather than to
  simply use the rolling strategy as proposed in the Chapter 3.}
