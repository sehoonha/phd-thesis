...
design agile 
motions on the hardware, such as delays, noises, \sehoon{``Noises'' plural?}
or safety concerns.
...
\revised{generate complex and stunning motions,}
and then transfer the solution to real hardware
...
In Chapter 3, we will show how to create a robust controller for generating 
...
\revised{that successfully reduce damage to body parts.}
...
% Goal, Method (Strength), Verification + Image
In Chapter 6, we will describe a new optimization algorithm for
\revised{highly constrained problems that usually happens when a user imposes a
  lot of instructions to a virtual character.}
...
% Goal, Method (Strength), Verification + Image
In Chapter 7, we will \sehoon{present tense? future?}
explain an evolutionary optimization algorithm for
...
we will test the proposed optimization algorithm for learning three parametrized
...
\begin{wrapfigure}{l}{0.5\textwidth}
 \vspace{-10pt}
  ...
  \label{fig:intro_bongo}
   \vspace{-10pt}
\end{wrapfigure}
In Chapter 8, we will describe an iterative approach for learning 
...
As a proof of concept, we will validate the algorithm on two different 
